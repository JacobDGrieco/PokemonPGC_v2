import { _decorateDexListIds } from './dex-ids.js';
import { _frontSprite, _frontSpriteAnimated, _frontSpriteShiny, _frontSpriteShinyAnimated } from './sprite-models.js';

export function _dex(game, type, id, form) {
  if (arguments.length === 3) {
    return { game, dexType: type, id };
  }
  return { game, dexType: type, id, form };
}

export const _regionalDex = (game, ...args) => _dex(game, 'regional', ...args);
export const _nationalDex = (game, ...args) => _dex(game, 'national', ...args);

export function formKeyToSuffix(natiId, formKey) {
...
export function _rebuildNatDexIndex() {
...
export function _registerDexData({
  baseKeys,
  dexName,
  variants,
  decorateIds = true,
  ...
  for (const baseKey of keys) {
    for (const v of vars) {
      ...
      if (decorateIds) list = _decorateDexListIds(dexKey, list);
  ...
  if (refreshNatIndex) _rebuildNatDexIndex();
}

export function _registerDexDataFromBaseDex({
  gen,
  baseKeys,
  dexName,
  baseDex,
  ...
  const dexList = Array.isArray(baseDex) ? baseDex : [];
  ...
  _registerDexData({
    ...
    buildDexFor(baseKey, dexKey) {
      const gameKey = String(baseKey || '').replace(/-national$/, '');
      const g = gen;

      return dexList.map((entry) => {
        if (!entry || typeof entry !== 'object') return entry;

        const natiId = entry.natiId ?? entry.id;
        const form = entry.form || null;
        const animated = Number(g) >= 5;

        return {
          ...entry,
          img: !entry.img
            ? (animated ? _frontSpriteAnimated(g, gameKey, natiId, form) : _frontSprite(g, gameKey, natiId, form))
            : entry.img,
          imgS: !entry.imgS
            ? (animated ? _frontSpriteShinyAnimated(g, gameKey, natiId, form) : _frontSpriteShiny(g, gameKey, natiId, form))
            : entry.imgS,
        };
      });
    },
