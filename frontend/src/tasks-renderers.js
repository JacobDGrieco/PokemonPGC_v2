...
function buildTaskItem(task, sectionId, setTasks, rootTasks, index, cbById) {
  ...
  item.className = `task-item ${isSub ? 'is-subtask' : 'is-main'}${!isSub ? (hasKids ? ' has-children' : ' no-children') : ''}${forceInline ? ' force-inline' : ''}${hasSlider ? ' has-slider' : ''}`;
  ...
  const labelHtml = isEitherTask(task)
    ? `<div class="small task-item-text task-either-title" data-id="${task.id}">${task.text}</div><div class="task-either-center">${eitherHtml}</div>`
    : `<input type="checkbox" ${task.done ? 'checked' : ''} /><div class="small task-item-text" data-id="${task.id}">${task.text}</div>`;

  if (isSub || (!hasKids && !forceInline)) {
    item.innerHTML = `${imgsHTML ? `<div class="task-item-img-wrap${(!isSub && !hasKids) ? '' : ''}">${imgsHTML}</div>` : ''}<label class="task-item-body ${isEitherTask(task) ? 'task-either-wrap' : ''}">${labelHtml}</label>`;
  } else {
    item.innerHTML = `<label class="task-item-body ${isEitherTask(task) ? 'task-either-wrap' : ''}">${labelHtml}${imgsHTML ? `<div class="task-item-img-wrap inline">${imgsHTML}</div>` : ''}</label>`;
  }
...
export function renderTaskList(tasks, sectionId, setTasks, allTasksRef, indexOpt, cbByIdOpt) {
  ...
  tasks.forEach((task) => {
    ...
    row.innerHTML = isEitherTask(task)
      ? `<div class="small task-item-text task-either-title" style="width:100%; text-align:center;">${task.text}</div><div class="task-either-center" style="width:100%;">${renderEitherHTML(task)}</div>`
      : `<input type="checkbox" ${task.done ? 'checked' : ''} /><div class="small" style="flex:1">${task.text}</div>`;
